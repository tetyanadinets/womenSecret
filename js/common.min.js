...
module.exports = /*#__PURE__*/function () {
  ...
  _createClass(Common, [{
    ...
    value: function select2Init() {
      ...
      $('.form-item_select .select-warehouses').select2({
        minimumResultsForSearch: Infinity
      });
      $('.form-item_select .select-city').select2({
        ajax: {
          ...
          processResults: function processResults(responce) {
            if (responce.data[0] && responce.data[0].Addresses && responce.data[0].Addresses.length) {
              return {
                results: $.map(responce.data[0].Addresses, function (val) {
                  if (val.Warehouses) {
                    return {
                      text: val.Present,
                      id: val.MainDescription
                    };
                  }
                })
              };
      ...
      $('.form-item_select .select-city').on('change', function () {
        $.ajax({
          url: 'https://api.novaposhta.ua/v2.0/json/',
          type: 'POST',
          dataType: "json",
          ...
          success: function success(responce) {
            if (responce.data && responce.data.length) {
              var data = $.map(responce.data, function (val) {
                return {
